...
def attention(x, start_pos, mask, freqs_cos, freqs_sin, attn_weights, args, cache_k, cache_v, dtype):
    ...
    xq = xq.reshape(B, L, n_local_heads, head_dim).astype(dtype)
    xk = xk.reshape(B, L, n_local_kv_heads, head_dim).astype(dtype)
    xv = xv.reshape(B, L, n_local_kv_heads, head_dim).astype(dtype)

    xq, xk = apply_rotary_emb(xq, xk, freqs_cos, freqs_sin)

    cache_k[:B, start_pos: start_pos+L] = xk
    cache_v[:B, start_pos: start_pos+L] = xv
    ks = cache_k[:B, :start_pos+L]
    vs = cache_v[:B, :start_pos+L]
    ...
    attention_float32 = (xq.astype(np.float32) @ xk.transpose(0,1,3,2).astype(np.float32)) / math.sqrt(head_dim)
    if mask is not None:
...
def transformer_block(x, start_pos, mask, freqs_cos, freqs_sin, block_weights, args, cache_k, cache_v, dtype):
    attn_weights, ff_weights, in_norm_weight, post_norm_weight = block_weights
    norm_x = rmsnorm(x, in_norm_weight, args.norm_eps, dtype)
    h1, cache_k, cache_v = attention(norm_x, start_pos, mask, freqs_cos, freqs_sin, attn_weights, args, cache_k, cache_v, dtype)
    z = (x + h1).astype(dtype)
...
def llama_init(model_path, args):
    ...
    freqs_cos, freqs_sin = compute_cos_sin_cache(args.dim // args.n_heads, args.max_seq_len, dtype=dtype)
    layer_blocks = []
    ...
    caches_k = [np.zeros((args.max_batch_size, args.max_seq_len,
                args.n_kv_heads if args.n_kv_heads else args.n_heads,
                args.dim // args.n_heads), dtype=dtype) for _ in range(args.n_layers)]
...
def llama_forward(model, input_ids, start_pos):
    ...
    freqs_cos = model["freqs_cos"][start_pos: start_pos + L]
    freqs_sin = model["freqs_sin"][start_pos: start_pos + L]

    mask = None
    if L > 1:
        mask = np.full((L, L), float("-inf"), dtype=dtype)
    ...
    for i, block in enumerate(model["layer_blocks"]):
        h, caches_k[i], caches_v[i] = transformer_block(
            h, start_pos, mask, freqs_cos, freqs_sin, block, args, caches_k[i], caches_v[i], dtype
        )

    h = rmsnorm(h, model["norm_weight"], args.norm_eps, dtype)
    logit = (h[:, [-1], :] @ model["lm_head_weight"].astype(np.float32)).astype(np.float32)
    return logit
...
def llama_generate(model, input_ids, max_new_tokens):
    ...
    for i in range(max_new_tokens):
        ...
        yield next_id
        curr_L += 1
        if curr_L >= model["args"].max_seq_len:
            break
