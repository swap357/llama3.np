...
class Attention:
    ...
    def __init__(self, q_weight: Array[D, D], k_weight: Array[D, D], v_weight: Array[D, D],
        ...
        if not Attention._debug_dtypes_printed_attention:
            ...
            for name, arr in [( "q_weight", self.q_weight), ("k_weight", self.k_weight),
                ...
                print(f"  {name: <10}: dtype={arr.dtype!s: <8}, shape={arr.shape!s: <25}, min={np.min(arr): .4f}, max={np.max(arr): .4f}, mean={np.mean(arr): .4f}")
    ...
    def __call__(self, x: Array[B, L or 1, D], start_pos: int, mask: Optional[Array[L, L]],
        ...
        if not Attention._debug_dtypes_printed_attention:
            ...
            print(f"  input x:      dtype={x.dtype!s: <8}, shape={x.shape!s: <25}, min={np.min(x): .4f}, max={np.max(x): .4f}, mean={np.mean(x): .4f}")
            if mask is not None:
                print(f"  input mask:   dtype={mask.dtype!s: <8}, shape={mask.shape!s: <25} (stats omitted due to -inf)")
            else:
                print("  input mask:   none")
            print(f"  freqs_cos:    dtype={freqs_cos.dtype!s: <8}, shape={freqs_cos.shape!s: <25}, min={np.min(freqs_cos): .4f}, max={np.max(freqs_cos): .4f}, mean={np.mean(freqs_cos): .4f}")
            print(f"  freqs_sin:    dtype={freqs_sin.dtype!s: <8}, shape={freqs_sin.shape!s: <25}, min={np.min(freqs_sin): .4f}, max={np.max(freqs_sin): .4f}, mean={np.mean(freqs_sin): .4f}")
        ...
        if not Attention._debug_dtypes_printed_attention:
            print(f"  qkv_proj xq:  dtype={xq_proj.dtype!s: <8}, shape={xq_proj.shape!s: <25}, min={np.min(xq_proj): .4f}, max={np.max(xq_proj): .4f}, mean={np.mean(xq_proj): .4f}")
            print(f"  qkv_proj xk:  dtype={xk_proj.dtype!s: <8}, shape={xk_proj.shape!s: <25}, min={np.min(xk_proj): .4f}, max={np.max(xk_proj): .4f}, mean={np.mean(xk_proj): .4f}")
            print(f"  qkv_proj xv:  dtype={xv_proj.dtype!s: <8}, shape={xv_proj.shape!s: <25}, min={np.min(xv_proj): .4f}, max={np.max(xv_proj): .4f}, mean={np.mean(xv_proj): .4f}")
        ...
        if not Attention._debug_dtypes_printed_attention:
            print(f"  qkv_reshape xq: dtype={xq.dtype!s: <8}, shape={xq.shape!s: <25}, min={np.min(xq): .4f}, max={np.max(xq): .4f}, mean={np.mean(xq): .4f}")
            print(f"  qkv_reshape xk: dtype={xk.dtype!s: <8}, shape={xk.shape!s: <25}, min={np.min(xk): .4f}, max={np.max(xk): .4f}, mean={np.mean(xk): .4f}")
            print(f"  qkv_reshape xv: dtype={xv.dtype!s: <8}, shape={xv.shape!s: <25}, min={np.min(xv): .4f}, max={np.max(xv): .4f}, mean={np.mean(xv): .4f}")

        # Apply RoPE
        xq_rope, xk_rope = apply_rotary_emb(xq, xk, freqs_cos, freqs_sin)

        if not Attention._debug_dtypes_printed_attention:
            print(f"  rope xq:      dtype={xq_rope.dtype!s: <8}, shape={xq_rope.shape!s: <25}, min={np.min(xq_rope): .4f}, max={np.max(xq_rope): .4f}, mean={np.mean(xq_rope): .4f}")
            print(f"  rope xk:      dtype={xk_rope.dtype!s: <8}, shape={xk_rope.shape!s: <25}, min={np.min(xk_rope): .4f}, max={np.max(xk_rope): .4f}, mean={np.mean(xk_rope): .4f}")
        ...
        if not Attention._debug_dtypes_printed_attention:
            print(f"  kv_cache write xk_rope (to k_cache): dtype={xk_rope.dtype!s:<8}, shape={xk_rope.shape!s:<25}, min={np.min(xk_rope):.4f}, max={np.max(xk_rope):.4f}, mean={np.mean(xk_rope):.4f}")
            print(f"  kv_cache write xv (to v_cache):    dtype={xv.dtype!s:<8}, shape={xv.shape!s:<25}, min={np.min(xv):.4f}, max={np.max(xv):.4f}, mean={np.mean(xv):.4f}")
            print(f"  k_cache overall:                  dtype={self.cache_k.dtype!s: <8}, shape={self.cache_k.shape!s: <25}")
            print(f"  v_cache overall:                  dtype={self.cache_v.dtype!s: <8}, shape={self.cache_v.shape!s: <25}")
        ...
        if not Attention._debug_dtypes_printed_attention:
            print(f"  kv_cache read ks: dtype={ks.dtype!s: <8}, shape={ks.shape!s: <25}, min={np.min(ks): .4f}, max={np.max(ks): .4f}, mean={np.mean(ks): .4f}")
            print(f"  kv_cache read vs: dtype={vs.dtype!s: <8}, shape={vs.shape!s: <25}, min={np.min(vs): .4f}, max={np.max(vs): .4f}, mean={np.mean(vs): .4f}")
        ...
        if not Attention._debug_dtypes_printed_attention:
            print(f"  gqa xk:       dtype={xk_gqa.dtype!s: <8}, shape={xk_gqa.shape!s: <25}, min={np.min(xk_gqa): .4f}, max={np.max(xk_gqa): .4f}, mean={np.mean(xk_gqa): .4f}")
            print(f"  gqa xv:       dtype={xv_gqa.dtype!s: <8}, shape={xv_gqa.shape!s: <25}, min={np.min(xv_gqa): .4f}, max={np.max(xv_gqa): .4f}, mean={np.mean(xv_gqa): .4f}")
        ...
        if not Attention._debug_dtypes_printed_attention:
            print(f"  transpose xq:   dtype={xq_transposed.dtype!s: <8}, shape={xq_transposed.shape!s: <25}, min={np.min(xq_transposed): .4f}, max={np.max(xq_transposed): .4f}, mean={np.mean(xq_transposed): .4f}")
            print(f"  transpose xk:   dtype={xk_transposed.dtype!s: <8}, shape={xk_transposed.shape!s: <25}, min={np.min(xk_transposed): .4f}, max={np.max(xk_transposed): .4f}, mean={np.mean(xk_transposed): .4f}")
            print(f"  transpose xv:   dtype={xv_transposed.dtype!s: <8}, shape={xv_transposed.shape!s: <25}, min={np.min(xv_transposed): .4f}, max={np.max(xv_transposed): .4f}, mean={np.mean(xv_transposed): .4f}")
        ...
        if not Attention._debug_dtypes_printed_attention:
            print(f"  scores_raw:     dtype={scores_raw.dtype!s: <8}, shape={scores_raw.shape!s: <25}, min={np.min(scores_raw): .4f}, max={np.max(scores_raw): .4f}, mean={np.mean(scores_raw): .4f}")
            print(f"  scores_scaled:  dtype={scores_scaled.dtype!s: <8}, shape={scores_scaled.shape!s: <25}, min={np.min(scores_scaled): .4f}, max={np.max(scores_scaled): .4f}, mean={np.mean(scores_scaled): .4f}")
        ...
        if mask is not None:
            ...
            if not Attention._debug_dtypes_printed_attention:
                if np.all(np.isfinite(current_mask_slice)):
                    print(f"  mask_slice:   dtype={current_mask_slice.dtype!s: <8}, shape={current_mask_slice.shape!s: <25}, min={np.min(current_mask_slice): .4f}, max={np.max(current_mask_slice): .4f}, mean={np.mean(current_mask_slice): .4f}")
                else:
                    print(f"  mask_slice:   dtype={current_mask_slice.dtype!s: <8}, shape={current_mask_slice.shape!s: <25} (contains non-finite)")
                print(f"  scores_masked:  dtype={scores_masked.dtype!s: <8}, shape={scores_masked.shape!s: <25}, min={np.min(scores_masked): .4f}, max={np.max(scores_masked): .4f}, mean={np.mean(scores_masked): .4f}")

        attention_weights: Array[B, HN, L_token, L_cache] = softmax(scores_masked)

        if not Attention._debug_dtypes_printed_attention:
            print(f"  attn_weights:   dtype={attention_weights.dtype!s: <8}, shape={attention_weights.shape!s: <25}, min={np.min(attention_weights): .4f}, max={np.max(attention_weights): .4f}, mean={np.mean(attention_weights): .4f}")

        # Output projection
        output_attended: Array[B, HN, L_token, HD] = attention_weights @ xv_transposed

        if not Attention._debug_dtypes_printed_attention:
            print(f"  output_attn:    dtype={output_attended.dtype!s: <8}, shape={output_attended.shape!s: <25}, min={np.min(output_attended): .4f}, max={np.max(output_attended): .4f}, mean={np.mean(output_attended): .4f}")
        ...
        if not Attention._debug_dtypes_printed_attention:
            print(f"  output_reshape: dtype={output_reshaped.dtype!s: <8}, shape={output_reshaped.shape!s: <25}, min={np.min(output_reshaped): .4f}, max={np.max(output_reshaped): .4f}, mean={np.mean(output_reshaped): .4f}")
            print(f"  final_output:   dtype={final_output.dtype!s: <8}, shape={final_output.shape!s: <25}, min={np.min(final_output): .4f}, max={np.max(final_output): .4f}, mean={np.mean(final_output): .4f}")
