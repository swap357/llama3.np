...
class RMSNorm:
    ...
    def __call__(self, x: Array[B, L or 1, D]):
        z_float32: Array[B, L or 1, 1] = (x.astype(np.float32) ** 2).mean(
            -1, keepdims=True
        ) + self.eps
...
class Attention:
    ...
    def __call__(
        ...
        attention_float32: Array[B, HN, L or 1, L] = (
...
class Llama:
    def __init__(self, model_path: str, args: ModelArgs):
        ...
        self.tok_embedding: Array[VS, D] = weight.get(
        ...
        self.norm = RMSNorm(
            weight.get("model.norm.weight"), eps=args.norm_eps, dtype=self.dtype
        )
        self.lm_head_weight: Array[D, VS] = weight.get("lm_head.weight").T.astype(
            self.dtype
        )
    ...
    def __call__(self, input_ids: Array[B, L], start_pos: int):
        ...
        if L > 1:
            ...
            if self.dtype is np.float16:
                mask = np.concatenate(
                    [np.zeros(zeros_shape, dtype=self.dtype), mask], axis=1
                )
            else:
                mask = np.concatenate([np.zeros(zeros_shape), mask], axis=1).astype(
                    self.dtype
                )
        ...
        logit: Array[B, 1, VS] = (
            h[:, [-1], :] @ self.lm_head_weight.astype(np.float32)
        ).astype(np.float32)
        return logit
