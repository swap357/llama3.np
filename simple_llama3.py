...
def apply_rotary_emb(xq, xk, freqs_cos, freqs_sin):
    ...
    xq_out = np.stack([xq_out_r, xq_out_i], axis=-1).reshape(xq.shape)
    xk_out = np.stack([xk_out_r, xk_out_i], axis=-1).reshape(xk.shape)
...
class Attention:
    ...
    def __call__(self, x, start_pos, mask, freqs_cos, freqs_sin):
        ...
        attn_scores = (xq @ k_seq.transpose(0, 1, 3, 2)) / math.sqrt(self.head_dim)
        if mask is not None:
...
if __name__ == '__main__':
    ...
    model = SimpleLlama("./stories15M.model.npz", args)
